...
void test1() {
    ...
    // empty square brackets mean lambdas are not capturing anything in capture list
...
void test3() {
    ...
    auto l4 = [] (Person p) {           // uses copy constructor
        std::cout << p << std::endl;
        p.set_name("Gus");
        p.set_age(40);
        std::cout << p << std::endl;
...
// std::function as a parameter for test 4
void filter_vector(const std::vector<int> &vec, std::function<bool(int)> func) { // function expects an int and returns a boolean
    std::cout << "[ ";
    for (int i : vec) {
        if (func(i))    // predicate function
            std::cout << i << " ";
    }
    std::cout << "]" << std::endl;
}

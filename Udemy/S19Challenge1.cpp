...
int main()
{
    ...
    std::cout << std::setw(51) <<  tours.title << std::endl;
    std::cout << std::endl;
    ...
    for(auto country : tours.countries) {   // loop through the countries
        // std::cout << std::setw(20) << std::left << std::setfill(' ') << country.name;
        ...
        for (size_t i = 0; i < country.cities.size(); i++) {
            std::cout   << std::setw(20) << std::left << ((i == 0) ? country.name : "")     // conditional to display empty space if not index 0
